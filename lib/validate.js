...
var validateAuth = function (auth) {
    if (auth === null || typeof auth === 'undefined')
        throw new Error('Null or undefined authentication data');
    if (typeof auth !== 'object')
        throw new Error('Authentication data must be an object');
    auth.Value = validateString(auth.Value, 'Value');
    auth.T3Token = validateString(auth.T3Token, 'T3Token');
}
module.exports.validateAuth = validateAuth;

module.exports.validateCredentials = function (credentials) {
    validateAuth(credentials);

    credentials.DashboardCookies = validateString(credentials.DashboardCookies, 'DashboardCookies');
    credentials.DashboardEndpoint = validateString(credentials.DashboardEndpoint, 'DashboardEndpoint');
}
